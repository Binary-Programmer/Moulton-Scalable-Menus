...
/**
 * A panel to serve as a parent component and backdrop for other MenuComponents. When you want to
 * add a menu component to this, simply call the component's constructor, which will either call
 * {@link #addToGrid(MenuComponent, int, int)} if the component is to reside in a gridded fashion,
 * or will add itself to this Panel's {@link #comps}.
 * <p>
 * One can use the {@link #createRoot(Color)} method to create a simple menu panel. For any panel,
 * {@link #setColor(Color)} can be used to set its backdrop color. Other methods like
 * {@link #setOutline(boolean)}, and {@link #setTextResize(Boolean)} can further be used to
 * customize the Panel. The GridFormatter (saved as {@link #grid}) is in charge of formatting the
 * details of the grid, such as margin, frame, and column or row weights.
 * 
 * @author Matthew Moulton
 */
public class Panel extends MenuComponent {
	/** The expressions to define the dimensions of this panel on its parent panel. */
	protected Expression width, height;
	/**
	 * The background color of this panel. A rectangle filled with this color is rendered unless it
	 * is null.
	 * 
	 * @see #setColor(Color)
	 * @see #getColor()
	...
	/**
	 * A list of the components held in free-form. The coordinates of the components held here are
	 * found by using this panel's coordinates at render time and then solving the algebraic
	 * expressions in the component that defines its location.
	 */
	protected LinkedList<MenuComponent> comps = new LinkedList<>();

	/**
	 * The formatter for the grid-based components held by this panel.
	 * <p>
	 * Components can be added to the grid by {@link #addToGrid(MenuComponent, int, int)} and
	 * removed by {@link #removeFromGrid(int, int, boolean)}, though {@link MenuComponent} handles
	 * this procedure automatically for construction and parent setting.
	 */
	protected GridFormatter grid = new GridFormatter();
	/**
	 * Whether or not text components should increase in size relative to height. When text size
	 * must be determined, if this panel has not set its value (defaults to null), it will defer to
	 * its parent panel. If no preference has been set, text will not resize relatively.
	 * 
	 * @see #textResize
	...
	/**
	 * The height of the panel (in pixels) at last render. Used by {@link MenuComponent} to know how
	 * much the text should resize for child components. Also used to tell child components their
	 * maximum heights
	 */
	protected int lastHeight = 0;
	/**
	 * The width of the panel (in pixels) at last render. Used to tell child components their
	 * maximum widths.
	 */
	protected int lastWidth = 0;

	/**
	 * @param parent the panel this panel will reside upon. Null if this is being set to
	...
	/**
	 * Creates a root panel. This is the same as calling {@link #Panel(Panel, int, int, Color)} with
	 * null, 0, 0, and background respectively but sometimes that can look confusing. For
	 * readability, this can be used. For the menu to be usable, the root panel must be saved as
	 * {@link MenuManager#menu}.
	 * 
	...
	public void render(Graphics g, int xx, int yy, int ww, int hh) {
		...
		try {
			...
			for (MenuComponent mc : comps) {
				if (mc != null && mc.isVisible()) { // free components are drawn anywhere in this
													// panel.
					mc.render(g, x, y, w, h);
				}
			}
	...
	/**
	 * Although Panel tells its children components to render where they truly are on the screen,
	 * offset is allowed to be use in subclasses. For example, {@link VirtualPanel} uses a subimage
	 * which components are drawn on, and that subimage is later drawn onto the screen. Children
	 * components may need to get their true location on the screen, for use in mouse touch and
	 * clicking boundaries for example.
	 * 
	 * @param comp the companion to find the render offset for
	 * @return a four element long pixel array for interpreting the render coordinates in terms of
	 *         real locations: <br>
	 *         <b>{ xStart, yStart, xEnd, yEnd }</b> <br>
	 *         The first two indices in the array are how far from where the component was told to
	 *         render the actual render location was. Therefore, if the panel told the component to
	 *         render at (0,0), but it was actually being rendered at (350,20), the first two values
	 *         in the returned array would be 350 and 20 respectively.<br>
	 *         The third and fourth values in the returned array signify ending offsets or cut offs.
	 *         If a component is told to render in the rectangle between (0,0) and (100,80), but
	 *         only 40 pixels of width and 50 pixels of height are actually being drawn to the
	 *         screen, then the third and fourth indices would be 40 and 50 respectively. The
	 *         minimum supported cut off value would be 0 (thus having no true width/height), and
	 *         negative values are to be interpreted as there being no cut off.
	 */
	...
	/**
	 * It is very frequent that components need their true coordinates for action in relation to
	 * the mouse (such as in mouse clicking or scrolling) or other uses. The coordinates that the
	 * component is told for rendering are not necessarily correct. Thus
	 * {@link #getRenderOffset(MenuComponent)} can be called to get the offsets that define how far
	 * the x and y components are from their true values, and if any cutting off occurred. For
	 * single coordinate queries, it is likely easier to calculate from the render offset method,
	 * but when a larger number of points need to be handled, this method can be used.
	 * 
	 ...
	 * @return a two dimensional array where the modified xs are in index 0, and the modified ys
	 * 		   are in index 1. Therefore, {xs, ys}.
	 */
	...
	/**
	 * Called by the constructor of supported MenuComponents to add the component to this panel's
	 * grid. Once the component has been added, the panel will take care of rendering and placing
	 * the component.
	 *
	...
	/**
	 * @return if text should resize by height. This is found by seeing if this panel has set a
	 *         preference in {@link #textResize} If a preference has been set, that is returned.
	 *         Otherwise, the parent is queried to see if it has been set.
	 */
	@Override
	...
	/**
	 * Returns a list of all of the child components that this panel holds. By default,
	 * {@link #grid} is added to the list, then {@link #comps}.
	 * 
	...
	/**
	 * Returns the formatter for this panel's grid components. Very useful to set margins, borders,
	 * or weights.
	 * 
	...
	/**
	 * Recursive method to remove all held components from the specified manager's list of touch
	 * responsive components. This is very useful if this panel will no longer be used or shown.
	 * 
