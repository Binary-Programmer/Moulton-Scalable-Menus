...
/**
 * The Form Button is a button specially adapted to work with forms. It will hold the focus of the
 * Menu Manager and receive text input. If the user presses enter ('\n'), the FormButton will
 * recognize the event as if it was being pressed by a mouse click.
 * @author Matthew Moulton
 */
public class FormButton extends Button implements TextInputComponent {
	/**The button needs to keep track of the menu manager to be able to initiate phantom click
	 * events when appropriate text input is received.*/
	...
	/**
	 * The form button does not actually keep track of any message or text, but it needs to receive
	 * updates from the menu manager when it has focus in order to receive enter events.
	 * @param string the string that would typically be appended, but here is compared against '\n'
	 * to identify an enter event.
	 */
	@Override
